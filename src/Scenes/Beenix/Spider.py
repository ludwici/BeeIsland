...
class Spider(pygame.sprite.Sprite):
    __slots__ = ("area", "image", "alive", "__velocity_x", "__velocity_y", "rect", "mask")
    ...
    def __collide_x(self, sprite) -> bool:
    ...
    def __collide_y(self, sprite) -> bool:
    ...
    def update(self) -> None:
        ...
        if self.alive:
            if self.__collide_x(self.area):
                self.vel_x *= -1
            if self.__collide_y(self.area):
                self.vel_y *= -1
        else:
            if self.__collide_x(self.area.area_inverse):
                self.vel_x *= -1
            if self.__collide_y(self.area.area_inverse):
                self.vel_y *= -1

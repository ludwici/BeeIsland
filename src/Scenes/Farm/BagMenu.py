...
class BagMenu(Menu):
    ...
    def __init__(self, parent: Scene) -> None:
        ...
        self.name_res_label.set_position((self.name_res_label.position[0] + self.resource_list_view.size[0] / 4
                                          - self.name_res_label.size[0] / 2, self.name_res_label.position[1]))
        ...
        self.amount_res_label.set_position(
            (self.amount_res_label.position[0] + self.resource_list_view.size[0] / 2
             + self.amount_res_label.size[0] / 2, self.amount_res_label.position[1])
        )
        self.can_show_hint = False
        self.sec_to_show = 2
        self.hint_text = ""
        bag_copy = copy(self.parent.player.resources.bag)
    ...
    def update(self, dt: float) -> None:
        if self.can_show_hint:
            ...
            if self.sec_to_show <= 0:
                self.stop_hint()
                self.parent.remove_render(self.parent.find_render_by_type(PopupNotify))
    ...
    def add_resource_to_list(self, r: Resource, index: int) -> None:
        ...
        r_label.set_position(
            (r_label.position[0] + 14, r_label.position[1] + i.size[1] / 2 - r_label.size[1] / 2))
        amount_label.set_position(
            (i.position[0] + i.size[0] - amount_label.size[0] - 28, r_label.position[1]))
